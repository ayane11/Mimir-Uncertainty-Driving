...
class MimirConfig:
    ...
    # GRPO after-training. The checkpoint_path on MimirAgent is the Mimir IL checkpoint.
    grpo: bool = False
    grpo_sample_time: int = 8
    grpo_num_samples_per_anchor: int = 1
    grpo_step_num: int = 2
    grpo_timestep_span: int = 20
    grpo_trunc_timestep: int = 8
    grpo_bc_coeff: float = 0.1
    grpo_use_bc_loss: bool = True
    ...
    grpo_logprob_clip_min: float = -5.0
    grpo_logprob_clip_max: float = 2.0
    grpo_clip_advantage_lower_quantile: float = 0.0
    grpo_clip_advantage_upper_quantile: float = 1.0
    grpo_scorer_config: PDMScorerConfig = field(
